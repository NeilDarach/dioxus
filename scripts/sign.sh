...
TEMP="$(mktemp -d -t com.DanTheMan827.AppSigner.XXXXX)"
OUTPUT="$TEMP/out"
mkdir "$OUTPUT" 2>&1
CURRENT_PATH="$(pwd)"
...
deb)
    ...
    mkdir "$TEMP/deb" 2>&1
    cd "$TEMP/deb" 2>&1
    ar -x "$FilePath" >/dev/null 2>&1 || (
        echo "Error extracting .deb"
        exit 1
    )

    tar -xf $TEMP/deb/data.tar* 2>&1 || (
        echo "Error untarring .deb data file"
        exit 1
    )
